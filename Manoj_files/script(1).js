...
function convertToPastTense() {
  const elements = document.querySelectorAll(
    ".paragraph, .spider-tag, .birthday-area"
  );
  const replacements = {
    "\\bis\\b": "was",
    "\\bare\\b": "were",
...
function calculateAge() {
  const birthday = new Date(2000, 3, 17); // March 23, 2002 (month is 0-indexed)
  const today = new Date();
...
function setupMultiverseGlitch(chapter) {
  const glitchPolaroids = chapter.querySelectorAll(
...
function setupSpiderTag3D() {
  ...
  if (card && container) {
    container.addEventListener("mousemove", (e) => {
      ...
      card.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateZ(10px)`;
      card.style.boxShadow = `${-mouseX / 20}px ${
        -mouseY / 20
      }px 30px -10px rgba(0,0,0,0.3)`;
    });
